...
import { PlusIcon, FilterIcon, EditIcon, DeleteIcon } from '../components/icons';
...
const HomePage: React.FC<HomePageProps> = ({ students, setStudents, tableView, setTableView }) => {
  ...
  const addStudent = (student: Omit<Student, 'id'>) => {
    return new Promise<void>((resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        if (student.nama.toLowerCase().includes("error")) { 
            setIsLoading(false);
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      ...
      <header className="text-white text-center py-12 px-6 rounded-2xl mb-8 shadow-lg bg-[var(--primary-color)]">
          <h1 className="text-4xl md:text-5xl font-bold mb-2">Manajemen Data Siswa 7C</h1>
          <p className="text-lg opacity-90">Selamat datang di dasbor manajemen siswa.</p>
      </header>
      
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div className="flex flex-col sm:flex-row gap-4 flex-grow">
            <button onClick={() => setAddModalOpen(true)} className="flex-1 flex items-center justify-center gap-2 bg-[var(--primary-color)] text-white font-semibold py-3 px-6 rounded-lg hover:brightness-110 transition-transform transform hover:scale-105 shadow-md">
              <PlusIcon />
              Tambah Siswa
            </button>
            <button onClick={() => setFilterModalOpen(true)} className="flex-1 flex items-center justify-center gap-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-semibold py-3 px-6 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-transform transform hover:scale-105 shadow-md">
              <FilterIcon />
              Filter Siswa
            </button>
          </div>
        ...
        <StudentTable students={filteredStudents} onEdit={openEditModal} onDelete={deleteStudent} tableView={tableView} />
      </div>
...
const StudentTable = ({ students, onEdit, onDelete, tableView }: { students: Student[], onEdit: (student: Student) => void, onDelete: (id: string) => void, tableView: 'responsive' | 'table' }) => (
        ...
        <table className={`w-full text-sm text-left text-gray-500 dark:text-gray-400 responsive-table ${tableView === 'table' ? 'force-table-view' : ''}`}>
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" className="px-6 py-3">T.B</th>
                    <th scope="col" className="px-6 py-3">B.B</th>
                    ...
                    return (
                        <tr key={s.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td className="px-6 py-4 flex items-center gap-2 actions-cell">
                                <button onClick={() => onEdit(s)} className="p-2 text-blue-500 hover:text-blue-700 dark:hover:text-blue-300 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700"><EditIcon /></button>
                                <button onClick={() => onDelete(s.id)} className="p-2 text-red-500 hover:text-red-700 dark:hover:text-red-300 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700"><DeleteIcon /></button>
                            </td>
                        </tr>
                    );
                })}
         ...
         {students.length === 0 && <p className="text-center p-8 text-gray-500 dark:text-gray-400">Tidak ada data siswa yang cocok.</p>}
    </div>
);
...
const AddStudentModal = ({ isOpen, onClose, onAdd, isLoading }: { isOpen: boolean, onClose: () => void, onAdd: (student: Omit<Student, 'id'>) => Promise<void>, isLoading: boolean }) => {
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newStudent = {
            ...
            ssw: (formData.getAll('ssw') as string[]).join(', '),
            tb: parseInt(formData.get('tb') as string, 10),
            bb: parseInt(formData.get('bb') as string, 10),
        };
        try {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Tambah Siswa Baru">
            <form onSubmit={handleSubmit} className="space-y-4">
                <FormInput id="nama" name="nama" label="Nama" type="text" required />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <FormInput id="umur" name="umur" label="Umur" type="number" required />
                    <FormInput id="tb" name="tb" label="Tinggi (cm)" type="number" required />
                </div>
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <FormSelect id="kelas" name="kelas" label="Kelas" required>
                        <option value="">Pilih Kelas</option>
                        {KELAS_OPTIONS.map(k => <option key={k} value={k}>{k}</option>)}
                    </FormSelect>
                     <FormSelect id="gender" name="gender" label="Gender" required>
                        <option value="">Pilih Gender</option>
                        {GENDER_OPTIONS.map(g => <option key={g.value} value={g.value}>{g.label}</option>)}
                    </FormSelect>
                </div>
                <FormInput id="bb" name="bb" label="Berat (kg)" type="number" required />
                <FormSelect id="ssw" name="ssw" label="SSW (Pilih satu atau lebih)" multiple size={5}>
                    {SSW_OPTIONS.map(ssw => <option key={ssw} value={ssw}>{ssw}</option>)}
                </FormSelect>
                <button type="submit" disabled={isLoading} className="w-full text-white bg-[var(--primary-color)] hover:brightness-110 focus:ring-4 focus:outline-none focus:ring-[var(--primary-color)]/30 font-medium rounded-lg text-sm px-5 py-2.5 text-center disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:cursor-not-allowed">
